...
`活動申請書生成` 頁面使用內建 `活動申請書模板.docx`，也可上傳自訂 `.docx` 範本。可從行事曆帶入活動名稱、日期、地點與活動負責人，並手動調整聯絡電話、活動宗旨、預估人數、茶品、點心單價與用途說明。活動宗旨可依活動名稱使用 AI 生成，並會顯示實際調用的模型。

產生時會替換申請書模板中的活動基本資料，並依幹部名單更新工作分配表。預估人數與點心單價會同步更新器材清單、經費概算與經費來源金額。
